...
### 修复
- **单设备登录持续生效**：checkSessionValid 直接用 refreshSession 返回数据，initSessionMonitor 重新注册 interval（5秒间隔）
- **儿童矿物质不显示**：getRDI 查不到男/女时改查'通用'性别
- **方案通知优化**：每天只弹一次

---

## V2.2.13（家庭管理不再包含本人信息 — 2026-06-13）
...
## V2.2.12（家庭管理同步基本信息 — 2026-06-13）

### 改动
- 修改家庭成员（本人）时同步更新 basic_info + Supabase
- switchFamilyMember 切换时同步

---

## V2.2.11（修复基本信息同步 — 2026-06-13）

### Bug 修复
- syncAllFromSupabase 基本信息提到最前面，Auth 元数据直读
- 不受 getCurrentAccountId 返回 null 跳过

---

## V2.2.10（基本信息改用 Auth 元数据 — 2026-06-13）

### 改动
- syncBasicInfoToSupabase 主通路改为 Auth 元数据
- syncAllFromSupabase 优先读 Auth 元数据
