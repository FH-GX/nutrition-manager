...
function selectLowCarbProfileForPage(profileIndex) {
    ...
    // 如果 users[currentUser] 有数据但缺失 xiaResult，从 localStorage 重新计算
 ...
 */
function applyNewPlanInline(ratio) {
    const info = loadBasicInfo();
    if (!info || !info.height || !info.weight) {
        showToast('请先填写基本信息', 'error');
 ...
 */
function autoGenerateDailyPlan(silent) {
